...
/**생년월일 유효성 검사 함수 year:number, month:number, day:number ~*/
...
,
    /**성별 생년 주민등록번호 조합 만들기 year:string, day:string, gender:string, local:boolena (국내 true 해외 false)*/
    getIdCardNumber : (year,day,gender,local)=>{
        if(local){
            if(gender === "M"){ //남자 
                if(Number(year) > 2000){
                    return year+day+3
